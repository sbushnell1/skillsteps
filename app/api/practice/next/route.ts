...
const sysLines = [
  ...
  .join("\n");
    ...
    // - when mode=grade: grade the last answer and either return a new question or finish+tips
    const schema = {
      init: {
        type: "object",
        required: ["question"],
    ...
    // Build user prompt with context and prior attempts
    let userContent = "";
    if (mode === "init") {
      userContent =
    ...
    // ---- Parse and validate model output (no `any`) ----
    ...
    if (mode === "init") {
      if (!isInit(parsed)) {
        return new Response(JSON.stringify({ error: "Bad model response" }), {
          status: 502,
        });
      }
